...
func LoadCodec() {
	logger.Printf("LoadCodec for bolt...")

	if err := xprotocol.RegisterProtocol(ProtocolName, &boltProtocol{}); err != nil {
		panic(err)
	}
	logger.Printf("RegisterProtocol finished")

	if err := xprotocol.RegisterMapping(ProtocolName, &boltStatusMapping{}); err != nil {
		panic(err)
	}
	logger.Printf("RegisterMapping finished")

	if err := xprotocol.RegisterMatcher(ProtocolName, boltMatcher); err != nil {
		panic(err)
	}
	logger.Printf("RegisterMapping finished")
	logger.Printf("LoadCodec bolt finished")
}
